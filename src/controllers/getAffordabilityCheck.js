...
const affordabilityCheckErr = new prometheusClient.Counter({
  name: 'journey_upgrade_affordability_check_error_count',
  help: 'vod-ms-journey-upgrade affordability_check error'
});

exports.handler = async function getAffordabilityCheck(req, res, next) {
  Sentry.info('Beginning affordabilitycheck...', {}, sentryCategory);

  if (req.query.idType === 'MSISDN' && req.params.idNumber.substr(0, 1) === '0') {

    const replaceZero = '27';

    req.params.idNumber = replaceZero + req.params.idNumber.substr(1);
  }
